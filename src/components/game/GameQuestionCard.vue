...
<template>
  ...
  >
    <CardHeader>
      <CardDescription>{{
        t('game.question_card.question_number', { current: questionNumber, total: totalQuestions })
      }}</CardDescription>
      <CardTitle class="text-2xl">
        {{ question }}
      </CardTitle>
      <div class="mt-2">
        <Progress :model-value="(questionNumber / totalQuestions) * 100" class="w-full" />
      </div>
    </CardHeader>
    <CardContent class="space-y-4">
      <!-- Top 5 Candidates Table -->
      <div v-if="topCandidates && topCandidates.length > 0" class="space-y-2">
        ...
        <div class="bg-muted/50 rounded-lg p-3 space-y-2">
          ...
          >
            <div class="flex items-center gap-2">
              <span
                class="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center text-xs font-medium"
              >
                {{ index + 1 }}
              </span>
    ...
    <CardFooter class="flex gap-4">
      <Button class="flex-1 transition-playful" size="lg" @click="emit('answer', true)">
        {{ t('game.yes') }}
      </Button>
      <Button
