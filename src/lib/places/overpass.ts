 ...
 */
export async function getHeight(lat: number, lng: number, radius = 20): Promise<number | null> {
  try {
    const query = `
      [out:json];
      (
    ...
    if (data.elements && data.elements.length > 0) {
      for (const element of data.elements) {
        if (element.tags?.height) {
          const height = Number.parseFloat(element.tags.height)
          if (!isNaN(height)) {
            return height
          }
        }
...
export async function enrichWithHeight(
  ...
  if (descriptors.extratags?.height) {
    const height = Number.parseFloat(descriptors.extratags.height)
    if (!isNaN(height)) {
      return height
    }
  }
