-- Fix type mismatch in filter_candidates_with_history
-- Cast all numeric calculations to double precision

DROP FUNCTION IF EXISTS filter_candidates_with_history(UUID[], JSONB);
...
BEGIN
  ...
  IF centroid_geom IS NOT NULL THEN
    SELECT MAX(ST_Distance(geom::geography, centroid_geom::geography))::double precision INTO max_distance
    FROM filtered_candidates;

    -- Calculate spatial confidence with explicit casting
    IF max_distance IS NULL OR max_distance = 0 THEN
      spatial_score := 1.0::double precision;
    ELSIF max_distance <= 50000 THEN
      spatial_score := 1.0::double precision;
    ELSIF max_distance <= 200000 THEN
      spatial_score := (0.7 + (0.3 * (1 - (max_distance - 50000) / 150000)))::double precision;
    ELSIF max_distance <= 500000 THEN
      spatial_score := (0.3 + (0.4 * (1 - (max_distance - 200000) / 300000)))::double precision;
    ELSE
      spatial_score := (0.2 * (1 - LEAST((max_distance - 500000) / 5000000, 1)))::double precision;
    END IF;
  ELSE
    spatial_score := 0.5::double precision;  -- Default if no geom available
  END IF;
  ...
  SELECT
    ...
    fc.lng,
    fc.descriptors,
    spatial_score::double precision as spatial_confidence,
    spatial_score::double precision as composite_confidence,
    0.0::double precision as semantic_similarity  -- Not recalculated in this function
...
All calculations cast to double precision to avoid type mismatches.';

-- Also fix match_places to use double precision consistently
...
BEGIN
  ...
  SELECT
    ...
    p.game_count,
    p.geom,
    (1 - (p.embedding <=> query_embedding))::double precision as sem_similarity
  FROM places p
  ...
  -- Calculate spatial confidence score based on clustering with explicit casting
  IF max_distance IS NULL OR max_distance = 0 THEN
    spatial_score := 1.0::double precision;
  ...
  SELECT
    ...
    tc.descriptors,
    tc.game_count,
    tc.sem_similarity::double precision as semantic_similarity,
    spatial_score::double precision as spatial_confidence,
    ((tc.sem_similarity * 0.6 + spatial_score * 0.4))::double precision as composite_confidence
